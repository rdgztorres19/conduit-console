...
public class EventMediator : IEventMediator
{
        ...
        get
        {
            ...
            if (instance is null)
            {
                throw new InvalidOperationException(
                    "EventMediator is not initialized. Build a Conduit instance first (ConduitBuilder.Create().Build()).");
            }
            return instance;
        }
    }
        ...
        IConduit conduit)
    {
        ...
        // Try resolve ILogger<EventMediator> from the Conduit activator (DI); fallback to NullLogger
        try
        {
            _logger = (ILogger<EventMediator>)_handlerActivator.CreateInstance(typeof(ILogger<EventMediator>));
        }
        catch
        {
