...
using Conduit.Mqtt.Configuration;
using Conduit.Mqtt.Internal;
...
internal sealed class MqttConnection : IMqttConnection
{
        ...
        IMessageContext context)
    {
        foreach (var registration in _handlerRegistrations)
        {
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error dispatching message to handler {HandlerType} for topic {Topic}",
    ...
    {
        _logger.LogInformation(
            "Subscribing to {Count} registered handler(s) for connection '{ConnectionName}'",
