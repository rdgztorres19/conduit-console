...
Allen-Bradley PLC communication module for Conduit using ASComm IoT library.

## Overview

Conduit.AsComm provides seamless integration with Allen-Bradley PLCs through the ASComm IoT library from Automated Solutions. It supports attribute-based tag subscription discovery, automatic polling, and type-safe tag reading/writing.
...
// Read a tag
var temp = await connection.ReadTagAsync<float>("Sensor_Temp");
Console.WriteLine($"Temperature: {temp.Value}");

// Write a tag
await connection.WriteTagAsync("Setpoint", 75.5f);
...
[AsCommSubscribe("plc1", "Alarm_Status", pollingIntervalMs: 100, OnChangeOnly = true)]
...
```csharp
[AsCommSubscribe("plc1", "Sensor_Temperature", pollingIntervalMs: 100, OnChangeOnly = true)]
...
```csharp
[AsCommSubscribe("plc1", "Motor_Speed", pollingIntervalMs: 50, OnChangeOnly = true, Deadband = 0.5)]
...
```csharp
[AsCommSubscribe("plc1", "Production_Count", pollingIntervalMs: 1000, OnChangeOnly = true)]
...
```csharp
[AsCommSubscribe("plc1", "Machine_Running", pollingIntervalMs: 200, OnChangeOnly = true)]
...
[AsCommSubscribe("plc1", "Current_Batch", pollingIntervalMs: 2000, OnChangeOnly = true)]
...
[AsCommSubscribe("plc1", "Active_Recipe", pollingIntervalMs: 2000)]
