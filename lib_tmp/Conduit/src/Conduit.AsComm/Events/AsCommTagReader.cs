...
namespace Conduit.AsComm.Events;

/// <summary>
/// Tag reader implementation for AsComm (PLC) connections.
/// Used by the EventMediator to read tags specified in [AsCommRead] attributes.
/// </summary>
public class AsCommTagReader : ITagReader
{
    private readonly IAsCommConnection _connection;

    /// <summary>
    /// Initializes a new instance of the <see cref="AsCommTagReader"/> class.
    /// </summary>
    /// <param name="connection">The AsComm connection to use for reading.</param>
    public AsCommTagReader(IAsCommConnection connection)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var method = typeof(IAsCommConnection).GetMethod(nameof(IAsCommConnection.ReadTagAsync));
            if (method is null)
            {
                throw new InvalidOperationException("ReadTagAsync method not found on IAsCommConnection");
            }
